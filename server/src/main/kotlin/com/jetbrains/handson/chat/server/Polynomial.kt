...
class Polynomial(coeffs: Array<Int>) {
    ...
    fun solve(): Double {
        var finResult = 99999.0
        var intervalStart = -10000.0
        val intervalStep = 0.001

        var x = 0.0

        while(intervalStart < 10000.0){
            var result = this.invoke(intervalStart);

            if(kotlin.math.abs(result) < finResult){
                finResult = result
                x = intervalStart
            }
            intervalStart += intervalStep
        }
        return x
    }
